...
constexpr uint64_t LOGTEXT_DURATION_MS = 10000;
...
public:
	...
	void SetLogStartPosition(TTLogPosition_e startPos) {
		logPosition = startPos;
	}
	// How long a log line lasts on screen. Default is LOGTEXT_DURATION_MS
	void SetLogDurationTicks(uint64_t durationMS) {
		logDurationMS = durationMS;
	}
...
private:
	...
	uint64_t logDurationMS = LOGTEXT_DURATION_MS;
