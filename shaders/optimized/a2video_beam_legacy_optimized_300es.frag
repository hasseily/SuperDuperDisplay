highp uniform int ticks;
highp uniform int hborder;
highp uniform usampler2D VRAMTEX;
lowp uniform sampler2D a2ModesTex0;
lowp uniform sampler2D a2ModesTex1;
lowp uniform sampler2D a2ModesTex2;
lowp uniform sampler2D a2ModesTex3;
lowp uniform sampler2D a2ModesTex4;
highp uniform int specialModesMask;
highp uniform int monitorColorType;
mediump in vec2 vFragPos;
mediump out vec4 fragColor;
void main()
{
  bool tmpvar_1;
  tmpvar_1 = bool(0);
  highp uvec2 fragOffset_2;
  uvec2 tmpvar_3;
...
tmpvar_1 = bool(1);
    ...
    else
    {
      ...
      if (tmpvar_8)
      {
        highp uint tmpvar_34;
        tmpvar_34 = (tmpvar_5.y >> 4);
        highp uint tmpvar_35;
...
;
      ...
      else
      {
        ...
        if (tmpvar_8)
        {
          highp uint byteValNext_44;
          highp uint byteValPrev_45;
          if ((0 < monitorColorType))
          {
            vec4 tmpvar_46;
            if ((monitorColorType < 2))
...
;
        if (tmpvar_10)
        {
          ...
          if (tmpvar_8)
          {
            highp uint byteVal4_55;
            highp uint byteVal1_56;
            if ((0 < monitorColorType))
            {
              vec4 tmpvar_57;
              if ((monitorColorType < 2))
