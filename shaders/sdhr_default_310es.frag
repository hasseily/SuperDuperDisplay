...
void main()
{
    //fragColor = vec4(1);
    //return;
    vec2 tileSize = vec2(meshSize) / vec2(tileCount);
    ...
    vec2 textureSize2d = vec2(textureSize(tilesTexture[texIdx],0));
    ...
    fragColor = (t * tex * vTintColor + (1.0 - t) * vStencilFailColor) * (1.0 - float(iDebugNoTextures))
